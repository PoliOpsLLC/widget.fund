(function(window, document) {
    ...
    const { key, location, campaign, employer, local, org, height, width } = document.currentScript.dataset;
    container.srcdoc = `
        <script src="${process.env.SCRIPT_DOMAIN}/iframe.${process.env.RELEASE_NAME || window.version}.js"></script>
        <script>window.init(${JSON.stringify({ key, location, campaign, employer, local, org })})</script>
    `;
